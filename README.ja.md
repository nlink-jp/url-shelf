...
- メニュー内のグループ化と昇順・降順は Settings で選べる（フォルダ優先＝既定 / 実体優先 /
  名称のみ）
- メニューの表示名は拡張子を除いたファイル名そのもの。順序を固定したければファイル名に
  番号を付け（`01_…`）、**名称のみ**を選ぶ。数値は自然順で比較されるので `2_` は `10_` より前
- 既存エントリの URL・開き方・ブラウザ指定を変えたい場合は、メニューで **⌘ を押しながらクリック**

メニュー項目の修飾キー: **⌥** でその回だけ通常⇄プライベート反転、**⌘** で設定変更、
**⌥⌘** で Finder に表示。
