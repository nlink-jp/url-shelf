...
すべて Finder でもできます。記録をファイルとして持つのはそのためです。Shelf ウィンドウが
あるのは、Finder にできない唯一のこと——`.webloc` の**中**に入っている設定の編集——のためです。

- **Shelf…**（⌘E）でツリー＋インスペクタを開く。リネーム・移動・削除と、エントリごとの
  URL・開き方・ブラウザ指定
- **Add URL…**（⌘N）で 1 件追加。クリップボードの URL が初期値に入る
- メニューバーアイコンへの URL ドロップでドロップ先フォルダに追加
- 削除は**ゴミ箱へ移動**。誤操作は Finder から戻せる
- ツリー内で**ドラッグ操作**が可能。フォルダの中央に落とすと移動、行の上端/下端に落とすと
  並べ替え。ブラウザから URL をツリーに落とすと、落とした先のフォルダに直接登録される
- 並べ替えるとそのフォルダのファイル名が採番し直される（`010_`, `020_`, …）。
  並び順はファイル名そのものであり、位置を裏で記録する仕組みは持たないため
- リネームは並び順プレフィックスを保存するので、位置が飛ばない

アプリの起動中に Finder 側で変更しても自動的に反映されます（必要になった時点で棚を読み直す
ため）。編集機能を入れた理由と、意図的に見送ったものは
[ADR-0001](docs/ja/adr-0001-shelf-management.ja.md) を参照。
