...
# Hardened Runtime alone suffices. Launching browsers via NSWorkspace and reading
# a user-selected folder require no entitlement in a non-sandboxed app.
...
# is $(APP_NAME).app.
BREW_KIND      := cask
BREW_DESC      := Menu-bar shelf of URL notes kept as .webloc files, with private-window opening
BREW_NAME      := $(NAME)
